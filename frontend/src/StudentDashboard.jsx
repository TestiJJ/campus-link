...
export default function StudentDashboard() {
  ...
  const [marketUniversities, setMarketUniversities] = useState(() => getCachedData('universities', []));
  const [isCampusModalOpen, setIsCampusModalOpen] = useState(false);
  const [showMarketUniDropdown, setShowMarketUniDropdown] = useState(false);
  const [marketUniSearch, setMarketUniSearch] = useState('');
  const marketUniDropdownRef = useRef(null);
  ...
  const loadAllData = async () => {
    ...
    API.get('/me').then(res => {
      if (res.data) {
        ...
        if (u.university_name) {
          setUniversityName(u.university_name);
        }
        setProfileForm({
    ...
    API.get('/universities').then(res => {
      ...
      setMarketUniversities(fetched);
      setCachedData('universities', fetched);
    }).catch(() => {});
  ...
  useEffect(() => {
    const handleOutsideUniClick = (e) => {
      if (marketUniDropdownRef.current && !marketUniDropdownRef.current.contains(e.target)) {
        setShowMarketUniDropdown(false);
      }
    };
    if (showMarketUniDropdown) {
  ...
  }, [showMarketUniDropdown]);

  // Unique Institutions for Marketplace Filter
  const availableMarketInstitutions = useMemo(() => {
    const map = new Map();
    (marketUniversities || []).forEach(u => {
    ...
    // Supplement from products & services if any have university info not yet in list
  ...
  }, [marketUniversities, products, services]);

  const filteredMarketInstitutions = useMemo(() => {
    const q = (marketUniSearch || '').toLowerCase().trim();
    if (!q) return availableMarketInstitutions;
    return availableMarketInstitutions.filter(u =>
      (u.name && u.name.toLowerCase().includes(q)) ||
      (u.abbreviation && u.abbreviation.toLowerCase().includes(q))
    );
  }, [availableMarketInstitutions, marketUniSearch]);

  const matchesUniversityFilter = (item) => {
    if (selectedUniversityFilter === 'all') return true;
    const userUniId = currentUser?.university_id;
    const userUniName = (currentUser?.university_name || currentUser?.university || '').toLowerCase().trim();
    const userUniAbbr = (currentUser?.university_abbr || '').toLowerCase().trim();

    if (selectedUniversityFilter === 'my_campus') {
      if (userUniId && item.university_id) {
        return String(item.university_id) === String(userUniId);
      }
      const itemUniName = (item.university_name || '').toLowerCase().trim();
      const itemUniAbbr = (item.university_abbr || '').toLowerCase().trim();
      const itemLoc = (item.vendor_location || item.location || item.dispatch_location || '').toLowerCase().trim();

      if (userUniAbbr && (itemUniAbbr === userUniAbbr || itemLoc.includes(userUniAbbr))) return true;
      if (userUniName && (itemUniName.includes(userUniName) || userUniName.includes(itemUniName) || itemLoc.includes(userUniName))) return true;
      return false;
    }

    // Filter by specific university ID or name/abbreviation
    if (item.university_id && String(item.university_id) === String(selectedUniversityFilter)) {
      return true;
    }
    const target = String(selectedUniversityFilter).toLowerCase().trim();
    const itemUniName = (item.university_name || '').toLowerCase().trim();
    const itemUniAbbr = (item.university_abbr || '').toLowerCase().trim();
    const itemLoc = (item.vendor_location || item.location || item.dispatch_location || '').toLowerCase().trim();
  ...
  return (
    <div className="h-dvh-screen max-h-dvh-screen overflow-hidden bg-slate-50 text-slate-900 font-sans antialiased flex flex-col select-none">
      ...
      <main className={`flex-1 w-full min-w-0 max-w-full flex flex-col min-h-0 h-full overflow-x-hidden overscroll-x-none ${activeTab === 'messages' ? 'overflow-hidden p-0' : 'overflow-y-auto p-0'}`}>
            ...
            <div className="space-y-3 mb-4">
              ...
              <CampusSelectModal
                ...
                onSelectUniversity={(uniKey, uniDisplayName) => {
                  setSelectedUniversityFilter(uniKey);
                  if (uniKey === 'all') {
                ...
                universities={availableMarketInstitutions}
                currentUserUniversity={{
                  name: currentUser?.university_name || currentUser?.university,
                  abbr: currentUser?.university_abbr,
                  id: currentUser?.university_id
                }}
              />
            </div>
              ...
              scatteredProducts.length > 0 ? (
                      ...
                      <div>
                        ...
                        <div className="aspect-square w-full bg-slate-100 relative overflow-hidden">
                          ...
                          <button
                            ...
                            onClick={(e) => {
                              e.stopPropagation();
                              const targetVal = p.university_id ? String(p.university_id) : (p.university_abbr || p.university_name);
                              if (targetVal) {
                                setSelectedUniversityFilter(targetVal);
                ...
                <div className="py-20 text-center bg-white rounded-3xl border border-slate-200 p-8 sm:p-10">
                  ...
                  <h4 className="text-base font-bold text-slate-800">
                    {selectedUniversityFilter !== 'all'
                      ? `No products found in ${getFilterDisplayLabel()}`
            ...
            {marketType === 'services' && (
              scatteredServices.length > 0 ? (
                ...
                <div className="py-20 text-center bg-white rounded-3xl border border-slate-200 p-8 sm:p-10">
                  ...
                  <h4 className="text-base font-bold text-slate-800">
                    {selectedUniversityFilter !== 'all'
                      ? `No services found in ${getFilterDisplayLabel()}`
        ...
        {activeTab === 'reels' && (
          <div className="max-w-2xl mx-auto space-y-3.5 sm:space-y-4">
            {/* Campus Drop Composer (Clean single bar) */}
            ...
            {/* Campus Marketplace Spotlight Rail (Featured Campus Drops & Services) */}
            {(products.length > 0 || services.length > 0) && (
              <div className="bg-white rounded-3xl border border-slate-200/80 p-3 sm:p-4 shadow-xs">
                <div className="flex items-center justify-between mb-3 px-1">
                  <div className="flex items-center space-x-2">
                    <span className="w-7 h-7 rounded-xl bg-amber-50 text-amber-600 flex items-center justify-center text-sm shadow-2xs">
                      🛍️
                    </span>
                    <div>
                      <h3 className="text-xs sm:text-sm font-black text-slate-900 tracking-tight">
                        Campus Marketplace Drops
                      </h3>
                ...
                <div className="flex items-stretch space-x-3 overflow-x-auto scrollbar-none momentum-scroll py-1 px-1">
                  ...
                  {rotatingHomeProducts.slice(0, 8).map((p) => (
                    <div
                      ...
                      className="w-36 sm:w-40 bg-slate-50 hover:bg-slate-100/90 rounded-2xl border border-slate-200/80 p-2 shrink-0 cursor-pointer group transition-all duration-200 flex flex-col justify-between"
                    >
                      <div>
                        <div className="aspect-square w-full rounded-xl overflow-hidden bg-slate-200 relative mb-2">
                          <SafeImage
                            src={p.image}
                            alt={p.name}
                        ...
                        <p className="text-[10px] text-slate-500 truncate mt-0.5">By {p.vendor_name}</p>
                      </div>

                      <div className="mt-2 pt-1 border-t border-slate-200/60 flex items-center justify-between">
                        <span className="text-[9px] font-bold text-sky-700 bg-sky-50 px-1.5 py-0.5 rounded-md truncate max-w-[80px]">
                          {p.university_abbr || 'Campus'}
                        </span>
                        <span className="text-[10px] font-bold text-sky-600 flex items-center">
                          Order
                        </span>
                      </div>
                    </div>
                  ...
                  {rotatingHomeServices.slice(0, 6).map((s) => (
                    ...
                    >
                      <div>
                        ...
                        <p className="text-[10px] text-slate-500 truncate mt-0.5">By {s.vendor_name}</p>
                      </div>

                      <div className="mt-2 pt-1 border-t border-slate-200/60 flex items-center justify-between">
                        <span className="text-[9px] font-bold text-indigo-700 bg-indigo-50 px-1.5 py-0.5 rounded-md truncate max-w-[80px]">
                          Service
                        </span>
                  ...
                  <div
                    ...
                    className="w-32 sm:w-36 rounded-2xl border-2 border-dashed border-sky-200 bg-sky-50/50 hover:bg-sky-50 p-4 shrink-0 cursor-pointer flex flex-col items-center justify-center text-center transition-all group active:scale-95"
                  >
                    <div className="w-10 h-10 rounded-full bg-white shadow-xs flex items-center justify-center text-sky-600 mb-2 group-hover:scale-110 transition-transform">
                      <ShoppingBag className="w-5 h-5" />
                    </div>
                    <span className="text-xs font-bold text-sky-800">More Deals</span>
                    <span className="text-[10px] text-sky-600 font-medium mt-0.5">Explore Market →</span>
                  </div>
                </div>
              </div>
            )}
            ...
            <div className="space-y-3.5 sm:space-y-4">
              {scatteredReels.map((reel, rIdx) => {
                    ...
                    return (
                      <React.Fragment key={reel.id}>
                        <div id={`reel-${reel.id}`} className={`bg-white rounded-3xl border ${highlightedReelId === reel.id ? 'ring-4 ring-sky-400 border-sky-500 shadow-xl' : 'border-slate-200/90'} overflow-hidden shadow-2xs hover:shadow-md transition-all duration-300`}>
                    {/* Post Header */}
              ...
              {scatteredReels.length === 0 && (
                <div className="space-y-4">
                  {/* Interactive Post Starter Banner */}
                  <div className="bg-gradient-to-r from-blue-600 via-indigo-600 to-sky-600 text-white rounded-3xl p-5 shadow-sm">
                    <div className="flex items-center space-x-3">
                      <div className="w-10 h-10 rounded-2xl bg-white/20 backdrop-blur-md flex items-center justify-center shrink-0 text-xl shadow-xs">
                        🚀
                      </div>
                      <div>
                        <h3 className="text-sm font-black text-white tracking-tight">
                          Start Today's Campus Wave!
                        </h3>
                        <p className="text-xs text-blue-100 mt-0.5">
                          Got a hostel gist, campus question, or photo? Be the first to share today.
                        </p>
                      </div>
                    </div>
                    <div className="mt-4 flex items-center gap-2 flex-wrap">
                      <button
                        type="button"
                        onClick={() => setQuickPostModalOpen(true)}
                        className="px-4 py-2 bg-white text-blue-700 font-bold text-xs rounded-xl shadow-xs hover:bg-blue-50 active:scale-95 transition-all cursor-pointer flex items-center space-x-1.5"
                      >
                      ...
                      <button
                        type="button"
                        onClick={() => reelFileInputRef.current?.click()}
                        className="px-3.5 py-2 bg-white/15 hover:bg-white/25 text-white font-semibold text-xs rounded-xl transition-all cursor-pointer flex items-center space-x-1.5"
                      >
                        <Camera className="w-4 h-4" />
                        <span>Share Photo/Video</span>
                      </button>
                    </div>
                  </div>

                  {/* Trending Marketplace Drops Feed (Ensures feed is never empty!) */}
                  {(products.length > 0 || services.length > 0) && (
                    <div className="space-y-3">
                      <div className="flex items-center justify-between px-1">
                        ...
                        <button
                          ...
                          className="text-xs font-bold text-sky-600 hover:text-sky-700 flex items-center space-x-0.5 cursor-pointer hover:underline"
                        >
                          ...
                          >
                            <div>
                              ...
                              <div className="p-2.5 sm:p-3">
                                ...
                                <span className="text-[10px] text-slate-500 font-medium block truncate mt-0.5">By {p.vendor_name}</span>
                              </div>
                            </div>
                            <div className="p-2.5 sm:p-3 pt-0 flex items-center gap-1.5">
                              <button
                                ...
                                className="flex-1 py-1.5 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold text-[10px] sm:text-[11px] rounded-lg transition-colors flex items-center justify-center space-x-1 cursor-pointer"
                              >
                              ...
                              <button
                                ...
                                className="flex-1 py-1.5 bg-sky-500 hover:bg-sky-600 text-white font-bold text-[10px] sm:text-[11px] rounded-lg transition-colors flex items-center justify-center space-x-1 cursor-pointer shadow-2xs active:scale-95"
                              >
                                <ShoppingBag className="w-3 h-3" />
                                <span>Order</span>
                              </button>
                      ...
                      <button
                        ...
                        className="w-full py-3 bg-gradient-to-r from-sky-500 to-blue-600 hover:from-sky-600 hover:to-blue-700 active:scale-98 text-white font-black text-xs rounded-2xl shadow-xs transition-all flex items-center justify-center space-x-2 cursor-pointer mt-2"
                      >
                        <ShoppingBag className="w-4 h-4" />
                        <span>Visit Full Campus Marketplace ({products.length} Products & Services)</span>
      ...
      {/* Campus & JAMB News Story Reader Modal */}
      <CampusNewsModal
